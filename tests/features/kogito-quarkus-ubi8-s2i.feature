...
Feature: kogito-quarkus-ubi8-s2i image tests
  ...
  Scenario: Verify if the s2i build is finished as expected using native build and runtime image
    Given s2i build https://github.com/kiegroup/kogito-examples.git from rules-quarkus-helloworld using 500.0.x and runtime-image quay.io/kiegroup/kogito-quarkus-ubi8:latest
      | variable | value |
      | NATIVE       | true       |
  ...
  Scenario: Verify if the s2i build is finished as expected using native build and no runtime image
    Given s2i build https://github.com/kiegroup/kogito-examples.git from rules-quarkus-helloworld using 500.0.x
      | variable | value |
      | NATIVE       | true       |
      | LIMIT_MEMORY | 3221225472 |
    Then check that page is served
  ...
  Scenario: Verify if the s2i build is finished as expected performing a non native build with runtime image
    Given s2i build https://github.com/kiegroup/kogito-examples.git from rules-quarkus-helloworld using 500.0.x and runtime-image quay.io/kiegroup/kogito-quarkus-jvm-ubi8:latest
      | variable | value |
      | NATIVE       | false                     |
  ...
  Scenario: Verify if the s2i build is finished as expected performing a non native build and if it is listening on the expected port , test uses custom properties file to test the port configuration.
    Given s2i build /tmp/kogito-examples from rules-quarkus-helloworld using 500.0.x and runtime-image quay.io/kiegroup/kogito-quarkus-jvm-ubi8:latest
      | variable | value |
      | NATIVE   | false |
    Then check that page is served
      | property        | value                 |
  ...
  Scenario: Verify if the s2i build is finished as expected performing a native build and if it is listening on the expected port, test uses custom properties file to test the port configuration.
    Given s2i build /tmp/kogito-examples from rules-quarkus-helloworld using 500.0.x and runtime-image quay.io/kiegroup/kogito-quarkus-ubi8:latest
      | variable | value |
      | NATIVE       | true       |
  ...
  Scenario: Verify if the s2i build is finished as expected performing a non native build with persistence enabled
    Given s2i build https://github.com/kiegroup/kogito-examples.git from process-quarkus-example using 500.0.x and runtime-image quay.io/kiegroup/kogito-quarkus-jvm-ubi8:latest
      | variable | value |
      | NATIVE            | false         |
  ...
  Scenario: Verify if the s2i build is finished as expected performing a native build with persistence enabled
    Given s2i build https://github.com/kiegroup/kogito-examples.git from process-quarkus-example using 500.0.x and runtime-image quay.io/kiegroup/kogito-quarkus-ubi8:latest
      | variable | value |
      | NATIVE            | true          |
      | LIMIT_MEMORY      | 6442450944    |
      | MAVEN_ARGS_APPEND | -Ppersistence |
  ...
  Scenario: Verify if the multi-module s2i build is finished as expected performing a non native build
    Given s2i build https://github.com/kiegroup/kogito-examples.git from . using 500.0.x and runtime-image quay.io/kiegroup/kogito-quarkus-jvm-ubi8:latest
      | variable | value |
      | NATIVE            | false                            |
  ...
  Scenario: Verify if the multi-module s2i build is finished as expected performing a native build
    Given s2i build https://github.com/kiegroup/kogito-examples.git from . using 500.0.x and runtime-image quay.io/kiegroup/kogito-quarkus-ubi8:latest
      | variable | value |
      | NATIVE            | true                             |
      | LIMIT_MEMORY      | 6442450944                       |
      | ARTIFACT_DIR      | rules-quarkus-helloworld/target  |
  ...
  Scenario: Perform a incremental s2i build
    Given s2i build https://github.com/kiegroup/kogito-examples.git from rules-quarkus-helloworld with env and incremental using 500.0.x
      | variable | value |
      | NATIVE   | false |
  ...
  Scenario: Verify that the Kogito Maven archetype is generating the project and compiling it correctly
    Given s2i build /tmp/kogito-examples from dmn-example using 500.0.x and runtime-image quay.io/kiegroup/kogito-quarkus-jvm-ubi8:latest
      | variable | value |
      | NATIVE         | false          |
      | KOGITO_VERSION | 500.0.1-SNAPSHOT |   
  ...
  Scenario: Verify that the Kogito Maven archetype is generating the project and compiling it correctly using native build
    Given s2i build /tmp/kogito-examples from dmn-example using 500.0.x and runtime-image quay.io/kiegroup/kogito-quarkus-ubi8:latest
      | variable | value |
      | NATIVE         | true           |
      | LIMIT_MEMORY   | 6442450944     |
      | KOGITO_VERSION | 500.0.1-SNAPSHOT |   
