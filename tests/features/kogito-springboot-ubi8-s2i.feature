...
Feature: kogito-springboot-ubi8-s2i image tests
  ...
  Scenario: Verify if the s2i build is finished as expected with debug enabled
    Given s2i build https://github.com/kiegroup/kogito-examples.git from process-springboot-example using 999.0.1 and runtime-image quay.io/kiegroup/kogito-springboot-ubi8:latest
      | variable | value |
  ...
  Scenario: Verify if the s2i build is finished as expected with no runtime image and debug enabled
    Given s2i build https://github.com/kiegroup/kogito-examples.git from process-springboot-example using 999.0.1
      | variable | value |
      | JAVA_OPTIONS        | -Ddebug=true |
    Then check that page is served
      | property             | value                                                                         |
  ...
  Scenario: Verify if the s2i build is finished as expected and if it is listening on the expected port, test uses custom properties file to test the port configuration.
    Given s2i build /tmp/kogito-examples from process-springboot-example using 999.0.1 and runtime-image quay.io/kiegroup/kogito-springboot-ubi8:latest
  ...
  Scenario: Verify if the s2i build is finished as expected using multi-module build with debug enabled
    Given s2i build https://github.com/kiegroup/kogito-examples.git from . using 999.0.1 and runtime-image quay.io/kiegroup/kogito-springboot-ubi8:latest
      | variable | value |
      | JAVA_OPTIONS      | -Ddebug=true                       |
  ...
  Scenario: Perform a incremental s2i build
    Given s2i build https://github.com/kiegroup/kogito-examples.git from process-springboot-example with env and incremental using 999.0.1
      # Leave those here as placeholder for scripts adding variable to the test. No impact on tests if empty.
      | variable | value |
    Then check that page is served
      | property             | value                                                                         |
  ...
  Scenario: Verify that the Kogito Maven archetype is generating the project and compiling it correctly when runtime is springboot
    Given s2i build /tmp/kogito-examples from dmn-example using 999.0.1 and runtime-image quay.io/kiegroup/kogito-springboot-ubi8:latest
      | variable | value |
